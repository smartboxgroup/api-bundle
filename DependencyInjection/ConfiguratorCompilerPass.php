...
class ConfiguratorCompilerPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        $contextRpc->addMethodCall('setApiConfigurator', [new Reference('smartapi.configurator')]);
        $contextRpc->addMethodCall('setServerBuilder', [new Reference('smartapi.soap.server.builder')]);
        $contextDocument->addMethodCall('setApiConfigurator', [new Reference('smartapi.configurator')]);
        $contextDocument->addMethodCall('setServerBuilder', [new Reference('smartapi.soap.server.builder')]);

        $complexTypeLoader = $container->getDefinition('besimple.soap.definition.loader.annot_complextype');
        $complexTypeLoader->addMethodCall('setSerializer', [new Reference('jms_serializer')]);
        ...
        if (!$container->hasDefinition($userProviderId)) {
            throw new InvalidConfigurationException(sprintf('No definition found for "%s" used in smartbox_api.userProvider configuration', $userProviderId));
        }
